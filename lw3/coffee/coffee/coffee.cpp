...
void DialogWithUser()
{
	cout << "Type 1 for coffee or 2 for black tea\n";
	int beverageChoice;
	cin >> beverageChoice;
...
int main()
{
	...
	cout << endl;
	{
		...
		auto latte = make_unique<CLatte>(CoffeeSize::Standart);
		// добавляем корицы
				...
				make_unique<CIceCubes>(				// | под нею - кубики льда
					make_unique<CLemon>(			// | | еще ниже лимон
						make_unique<CCinnamon>(		// | | | слоем ниже - корица
							make_unique<CLatte>(CoffeeSize::Double)),	// | | |   в самом сердце - Латте
						2),							// | | 2 дольки лимона
	...
	// Подробнее рассмотрим работу MakeCondiment и оператора <<
	{
		...
		*/
		...
		auto oneMoreLemonIceTea =
			make_unique<CTea>(TeaType::Matcha)	// Берем чай
	...
	{
		auto beverage =
			make_unique<CMilkshake>(MilkshakeSize::Large)					// Наливаем молочный коктейль
